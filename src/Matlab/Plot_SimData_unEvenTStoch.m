...
for ss = 1:length(setAll)
    ...
    for c = 1:length(allConventions)
        ...
        aucLinkSelcItr = zeros(numPat,2);
        aucLinkNoMuSelcItr = zeros(numPat,2);
        aucUnLinkSelcItr = zeros(numPat,2);
        aucUnLinkNoMuSelcItr = zeros(numPat,2);
        
        selcSitesAll = zeros(numPat,50);
        %====================== END INITIALIZATION ================================
...
%             disp(['Protein: ' thisProt])
        ...
        %--------------------------------------------------------------------------
                ...
                newDir = [dirNameDataTemp 'Set' num2str(thisSet) '/ng_' ngSamplingSchemeStr '_dT_' dTSamplingSchemeStr '_Tused_' tjSamplingSchemeStr '_initStr' num2str(numStrainsInInitialPop) '/MC' num2str(pat) '/synth/perSiteSelc/'];
...
%                 [~,~,~, aucUnLinkNoMuItrTemp(2)] = perfcurve(double(~negOnlyItrTemp), sigmaEstOutUnLinkNoMu, 1);
        ...
        if(exist([ResultFolder thisResultFile], 'file') == 2)
            ...
            delete([ResultFolder thisSetConvSelcSitesFile])
        end
        ...
        dlmwrite([ResultFolder thisSetConvSelcSitesFile], selcSitesAll)
