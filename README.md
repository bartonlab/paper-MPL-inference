...
\* Equal contributions  
<sup>#</sup> correspondence to [mailto:m.mckay@ust.hk](m.mckay@ust.hk) and [mailto:john.barton@ucr.edu](john.barton@ucr.edu)  
...
Due to the large size and number of some files generated by simulations and by the interim analysis of HIV-1 data, some data has been stored in a compressed format using Zenodo. To access the full set of data, navigate to the [https://zenodo.org/record/3332902](Zenodo record). Then download and extract the contents of the archives `src-MPL-HIV.tar.gz` and `src-wfsim-data.tar.gz` into the folders `src/MPL/HIV` and `src/wfsim/data`, respectively.
